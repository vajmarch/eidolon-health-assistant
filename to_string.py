import re
from flattener import flatten_ehr, flat_to_string, find_patient
...
def any_date_to_str(ehr_date, converter, parent_field=''):
    what = parent_field_to_str(parent_field, 'unknown')
    data_time = date_time_pattern1.findall(ehr_date)[0]
...
def ignore_to_str(ehr_value, converter, parent_field=''):
    return []
...
def default_to_str(ehr_value, converter, parent_field=''):
    if type(ehr_value) is dict:
        flat = flatten_ehr(ehr_value)
        return [f'{parent_field} {flat_to_string(flat)}']
    if type(ehr_value) is list:
        flat = flatten_ehr(ehr_value)
        return [f'{parent_field} {flat_to_string(flat)}']
    return [f'The {parent_field} for this {converter.doc} is {ehr_value}.']


def resource_type_to_str(ehr_value, converter, parent_field=''):
    return [f'The type of information in this {converter.doc} is {split_camel(ehr_value)}.']


def any_code_to_str(ehr_value, converter, parent_field=''):
    output = []
    what = parent_field_to_str(parent_field, 'code')
    if type(ehr_value) is str:
        output.append(f'The {what} for this {converter.doc} is {ehr_value}.')
    elif type(ehr_value) is dict and 'coding' in ehr_value:
        if len(ehr_value['coding']) == 1:
            code_value = ehr_value['coding'][0]
            ...
            for i, code_value in enumerate(ehr_value['coding']):
                if 'display' in code_value:
...
def any_value_quantity_to_str(ehr_value, converter, parent_field=''):
    what = parent_field_to_str(parent_field, 'quantity value')
    return [f'The {what} for this {converter.doc} is {ehr_value["value"]} {ehr_value["unit"]}.']


def any_value_string_to_str(ehr_value, converter, parent_field=''):
    what = parent_field_to_str(parent_field, 'value')
    return [f'The {what} for this {converter.doc} is {ehr_value}']


def any_value_codeable_concept_to_str(ehr_value, converter, parent_field=''):
    what = parent_field_to_str(parent_field, 'value or result')
    if 'display' in ehr_value["coding"][0]:
        return [f'The {what} for this {converter.doc} is {ehr_value["coding"][0]["display"]}.']
    if 'code' in ehr_value["coding"][0]:
        return [f'The {what} for this {converter.doc} is {ehr_value["coding"][0]["code"]}.']
    return []


def any_status_to_str(ehr_value, converter, parent_field=''):
    what = parent_field_to_str(parent_field, 'status')
    return [f'The {what} for this {converter.doc} is {ehr_value}.']


def any_category_to_str(ehr_value, converter, parent_field=''):
    output = []
    what = parent_field_to_str(parent_field, 'category')
    if type(ehr_value) is list:
        if len(ehr_value) == 1:
            category = ehr_value[0]
            ...
            for i, category in enumerate(ehr_value):
                if type(category) is dict and 'coding' in category:
                    for code_value in category['coding']:
                        if 'display' in code_value:
                            output.append(
        ...
        for code_value in ehr_value['coding']:
            for code_value in ehr_value['coding']:
                if 'display' in code_value:
                    output.append(f'The {what} of this {converter.doc} is {code_value["display"]}.')
                elif 'code' in code_value:
        ...
        output.append(f'The {what} of this {converter.doc} is {ehr_value}.')

    return output


def any_components_to_str(ehr_value, converter, parent_field=''):
    output = []
    what = parent_field_to_str(parent_field, 'component')
    output.append(f'This {converter.doc} contains {len(ehr_value)} {what}s.')
    for i, component in enumerate(ehr_value):
...
def any_reaction_to_str(ehr_value, converter, parent_field=''):
    output = []
    what = parent_field_to_str(parent_field, 'reaction')
    if type(ehr_value) is list:
        for i, reaction in enumerate(ehr_value):
...
def any_dosage_instruction_to_str(ehr_value, converter, parent_field=''):
    output = []
    what = parent_field_to_str(parent_field, 'dosage instruction')
    if len(ehr_value) == 1:
        if 'text' in ehr_value[0]:
            output.append(f'The {what} for this {converter.doc} is {text_to_str(ehr_value[0]["text"])}.')
    else:
        for dosage in ehr_value:
            if 'sequence' in dosage and 'text' in dosage:
                output.append(
                    f'The {ordinal(dosage["sequence"])} {what} for this {converter.doc} is {text_to_str(ehr_value["text"])}.')
    return output


def any_insurance_to_str(ehr_value, converter, parent_field=''):
    output = []
    what = parent_field_to_str(parent_field, 'insurance')
    if len(ehr_value) == 1:
        if 'coverage' in ehr_value[0] and 'display' in ehr_value[0]['coverage']:
            output.append(f'The {what} for this {converter.doc} is {ehr_value[0]["coverage"]["display"]}.')
    else:
        for dosage in ehr_value:
...
def any_claim_item_to_str(ehr_value, converter, parent_field=''):
    output = []
    what = parent_field_to_str(parent_field, 'item')
    if len(ehr_value) == 1:
        if 'productOrService' in ehr_value[0]:
            output += any_code_to_str(ehr_value[0]['productOrService'], converter, parent_field=what)
    else:
        for item in ehr_value:
            if 'sequence' in item and 'productOrService' in item:
                output += any_code_to_str(ehr_value[0]['productOrService'], converter,
                                          parent_field=f'{ordinal(item["sequence"])} {what} ')
    return output


def any_reference_to_str(ehr_value, converter, parent_field=''):
    # what = parent_field_to_str(parent_field, 'reference')
    # if 'display' in ehr_value:
    #     return [f'The {what} for this {converter.doc} is {ehr_value["display"]}.']
    # if 'reference' in ehr_value:
    #     return [f'The {what} for this {converter.doc} is {ehr_value["reference"]}.']
    return []


def any_class_to_str(ehr_value, converter, parent_field=''):
    what = parent_field_to_str(parent_field, 'class')
    if 'display' in ehr_value:
        return [f'The {what} for this {converter.doc} is {ehr_value["display"]}.']
    if 'code' in ehr_value:
        return [f'The {what} for this {converter.doc} is {ehr_value["code"]}.']
    return []


def any_simple_money_to_str(ehr_value, converter, parent_field=''):
    what = parent_field_to_str(parent_field, 'money')
    if 'value' in ehr_value and 'currency' in ehr_value:
        return [f'The {what} for this {converter.doc} is {ehr_value["value"]} {ehr_value["currency"]}.']
    if 'value' in ehr_value:
        return [f'The {what} for this {converter.doc} is {ehr_value["value"]}.']
    return []


def any_contained_to_str(ehr_value, converter, parent_field=''):
    output = []
    for i, resource in enumerate(ehr_value):
...
# explanation_of_benefit_field_to_str_dict['total'] = lambda ehr_value,parent_field='',doc='': ['TOTAL HERE!']
...
def ehr_bundle_to_strings(bundle):
    patient, patient_name = find_patient(bundle)
    patient_str = patient_to_str(patient)
    output = []
    for entry in bundle['entry']:
        ehr_str = ehr_to_string(entry['resource'])
        if ehr_str is not None:
            ehr_str = ' '.join(ehr_str)
